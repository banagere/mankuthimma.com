...
In the word-building game[^70], among the lot of letter-chits we select letters and assemble a word. So also a lonely person, searching for a companion becomes complete only when he gets one.
...
Teaching a variety of tastes, cooking delicious foods making the mouth water, sighing warm breaths, and invading from ten directions[^71] -- who would not fall prey to such a nature?

[^70]: We play the word-building game' with chits of paper with the various alphabets of the English language written on them - on every chit is an alphabet. For each round of play, we mix up fifteen sets of chits and make a heap. One by one, players pick up chits and then try to make a meaningful word out of the chits they have. For instance, if one picks up chits with the letters G, R, N, and B then she would pick the letter I in order to make the word bring'.
[^71]: In the Indian tradition, the ten directions are: purva (East), pascima (West), uttara
(North), daksina (south), isanya (Northeast), agneya (Southeast), vayavya (Northwest),
nairrtya (Southwest), ürdhva (upward, zenith), and adhah (downward, nadir)